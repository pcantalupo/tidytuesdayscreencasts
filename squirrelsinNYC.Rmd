...
# Squirrels in NYC
- [Video](https://www.youtube.com/watch?v=6GV9sAD6Pi0), [Stopped here](https://youtu.be/6GV9sAD6Pi0?t=1502)
...
nyc_squirrels %>%
  ...
  summarize_at(vars(long, lat, runs_from, indifferent), mean) %>% # summarizing multivars with same function
  ungroup() %>%
  filter(n>=10) %>%
  ...
  labs(color = "% of squirrels run", size = "# of squirrels",
       title = "S in NW of Central Park are more likely to run away")
```


Stopped at 25 min mark
